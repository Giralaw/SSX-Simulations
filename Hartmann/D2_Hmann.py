...
hartmann.substitutions['Bx'] = "Ay_z" # components, not derivatives
...
# Navier Stokes
hartmann.add_equation("dt(vx) + dx(p) - Lap(vx, vx_z)/Re = -vx*dx(vx) -vy*dy(vx) - vz*dz(vx) - Ha**2/(Re*Rm) * Jy*Bz - Pi*(exp(-t/tau) - 1)")
hartmann.add_equation("dt(vy) + dy(p) - Lap(vy, vy_z)/Re = -vx*dx(vy) -vy*dy(vy) - vz*dz(vy)")
hartmann.add_equation("dt(vz) + dz(p) - Lap(vz, vz_z)/Re = -vx*dx(vz) -vy*dy(vz) - vz*dz(vz) + Ha**2/(Re*Rm) * Jy*Bx")  #vx or vz for first RHS term?
